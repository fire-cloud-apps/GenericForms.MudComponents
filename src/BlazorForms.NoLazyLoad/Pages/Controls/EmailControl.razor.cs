...
public partial class EmailControl : ComponentBase
{
    ...
    private string cardTitle = "email";
    private async Task AllControlDetails()
    {
        ...
        if (_formAllControlBuilders is not null)
        {
            DynamicMudForm.AttachCard_EventAction
                (_formAllControlBuilders, cardTitle, CardActionClick);
            DynamicMudForm.AttachSubmitButton_EventAction
                (_formAllControlBuilders, cardTitle, SubmitButton_Click);
        }
